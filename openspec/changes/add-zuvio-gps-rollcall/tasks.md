...
## 2. 前端
- [x] 2.1 新增 `frontend/src/hooks/useZuvioAccounts.ts`（localStorage `zuvio_accounts`，含 `courseId`）與 `zuvio_location` 存取
- [x] 2.2 新增 `frontend/src/components/ZuvioPanel.tsx`：帳號 CRUD、GPS 座標欄（可貼 Google 地圖「緯度, 經度」字串）與「使用目前位置」、載入課程與下拉單選

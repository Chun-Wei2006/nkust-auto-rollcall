...
在「帳號管理」中點擊帳號列上的分享圖示（或用「分享多個」勾選多個帳號），會產生一條分享連結：

```
https://autorollcall.chunweidev.com/import#<payload>
```

- 帳號資料放在網址的 `#` 之後，瀏覽器不會把這段送到任何伺服器
- 手機上按「分享」會叫出系統分享單，可直接 AirDrop、Quick Share 或傳到 LINE；電腦上則退回「複製連結」
